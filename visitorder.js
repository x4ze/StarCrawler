...
export const crawling_queue = new Queue;
 ...
 */
export function addURLToQueue(url) {
 ...
 */
export function addURLArrayToQueue(url_array) {
...
export function hasVisited(url) {
    const lemmatized_url = lemmatizeURL(url);
 ...
 */
export function hasVisitedArray(url_array) {
    const result = [];
    for (let index = 0; index < url_array.length; index++) {
        const url = url_array[index];
        const has_visited = hasVisited(url);
 ...
 * @param url a URL as a string
 */
export function addToVisitedURLs(url) {
    const time = Date.now();
 ...
 */
export function lemmatizeURL(url_string) {
